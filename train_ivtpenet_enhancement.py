...
from monai.transforms import (
    Compose, RandFlipD, RandRotateD, RandZoomD, RandShiftIntensityD,
    RandGaussianNoiseD, RandAffineD, RandAdjustContrastD,
    NormalizeIntensityD, ScaleIntensityd, RandAdjustContrastd, RandGaussianNoised, RandGibbsNoised, RandBiasFieldd,
    RandGaussianSmoothd, RandShiftIntensityd, RandHistogramShiftd, RandCoarseDropoutd, RandCoarseShuffled,
    RandKSpaceSpikeNoised, RandZoomd, RandRotated, Rand3DElasticd, RandRotate
)
...
if __name__ == '__main__':
    ...
    model = IVT_PENet.Net(n_filters=[64, 96, 128, 160], en_blocks=[2, 3, 3, 3], de_blocks=[1, 2, 2],
    ...
    transform_list_train = [
        ...
        RandShiftIntensityd(keys=["image"], prob=0.2, offsets=(-3, 3)),
    ]
    transform_list_valid = []

    train_transform = Compose(transform_list_train)
    ...
    warm_up_epochs = int(0.15 * total_epochs)
    stable_epochs = int(0.60 * total_epochs)
    cool_down_epochs = total_epochs - warm_up_epochs - stable_epochs

    max_probs = {
        RandGaussianNoised: 0.3,
        RandShiftIntensityd: 0.2,
    }

    aug_schedules = {transform_type: [] for transform_type in max_probs.keys()}
    multiplier_schedule = []

    for e in range(total_epochs):
        current_multiplier = 0.0
        if e < warm_up_epochs:
            current_multiplier = e / warm_up_epochs if warm_up_epochs > 0 else 1.0
        elif e < warm_up_epochs + stable_epochs:
            current_multiplier = 1.0
        else:
            progress = (e - warm_up_epochs - stable_epochs) / cool_down_epochs if cool_down_epochs > 0 else 1.0
            current_multiplier = 0.5 * (1 + np.cos(np.pi * progress))

        multiplier_schedule.append(current_multiplier)

        for transform_type, max_p in max_probs.items():
            current_prob = max_p * current_multiplier
            aug_schedules[transform_type].append(current_prob)
    writer = SummaryWriter(options.log_path + unique_name)
    for i in range(0, options.epochs):

        for t in train_transform.transforms:
            transform_type = type(t)
            if transform_type in aug_schedules:
                t.prob = aug_schedules[transform_type][i]

        print('\nEpoch: {}'.format(i))
        print('current lr :', optimizer.param_groups[0]['lr'])
        print('current aug multiplier: {:.4f}'.format(multiplier_schedule[i]))
