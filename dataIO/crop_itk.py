...
def one_hot(a, num_classes):
    return np.eye(num_classes, dtype='uint8')[a.reshape(-1)].transpose(1, 0).reshape([num_classes] + list(a.shape))#将标签数组转换为one-hot编码形式
...
class SlideCrop(object):
    ...
    def __call__(self, sample):
        ...
        # 支持两种键名：image 或 image_itk
        img_key = 'image' if 'image' in sample else 'image_itk'
        image = sitk.GetArrayFromImage(sample[img_key]).astype('float32')
